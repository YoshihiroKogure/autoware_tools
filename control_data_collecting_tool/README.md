...
## Trajectory generation and data collection logic
  ...
  - ### Data collection logic common to all courses
    ...
    By appropriately adjusting the `target_velocity` provided to the pure pursuit algorithm, speed and acceleration data are efficiently collected.
    Data collection consists of four phases: selection of target speed and acceleration, acceleration phase, constant speed phase, the deceleration phase.


    1. Selection of target speed and acceleration

        In the speed-acceleration heatmap, the speed and acceleration with fewer data points are set as the target speed and acceleration, which are then defined here as `phase_target_velocity` and `phase_target_acceleration`.

    2. Acceleration phase
      The vehicle accelerates by setting `target_velocity` as follows until its speed exceeds phase_target_velocity.
      
        ```Python3
          target_velocity = current_velocity + abs(phase_target_acceleration) / acc_kp + sine_curve
        ```

        `current_velocity` is a current velocity of vehicle and `acc_kp` accel command proportional gain in pure pursuit algorithm. `sine_curve` is a sine wave added to partially prevent cases where the vehicle does not run with the target acceleration.

    3. Constant speed phase
      When the vehicle reaches `phase_target_velocity`, `target_velocity` is defined as follows to allow the vehicle to run around the target speed for a certain period of time.

        ```Python3
          target_velocity = phase_target_velocity + sine_curve
        ```
    ...
    4. Deceleration phase
      In the deceleration phase, similar to the acceleration phase, `target_velocity` is defined as follows to ensure the vehicle decelerates.
        ...
        After decelerating to a sufficiently low speed, return to step 1.
  ...
  - ### Trajectory generation and data collection logic specific to `reversal_loop_circle`
    ...
    - #### Trajectory generation logic for `reversal_loop_circle`
        ...
        The `reversal_loop_circle` is primarily generated by connecting the following three components, `common tangent`, `circumcircle` and `boundary` as shown in the following picture.
        ...
        - `circumcircle`
          The `circumcircle` part is created by drawing the common circumscribed circle of the circles.
          ...
          <img src="resource/circumcircle.png" width="480">
          <img src="resource/circumcircle_counter_clockwise.png" width="480">

        - `boundary`
          The `boundary` part is used to connect the `common tangent` and the `circumcircle` sections.
    ...
    - #### Velocity and steering angle data collection logic for `reversal_loop_circle`
      - Velocity and steering angle
        Speed and steering angle data are collected in the common tangent section. From the current position of the ego vehicle, we look ahead along the trajectory for a distance of looking_ahead_distance to find the maximum curvature.
        The maximum curvature is used to calculate the steering angle the vehicle will take, and the vehicle accelerates or decelerates towards the speed with the least amount of data for that steering angle.
        The `common tangent` section efficiently collects steering angle data because the trajectory with the curvature that has the least data is generated by adding a sine curve with an appropriate amplitude.
