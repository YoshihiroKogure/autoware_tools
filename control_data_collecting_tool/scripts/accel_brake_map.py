...
class MapConverter:
    def __init__(self, path_to_map):
        ...
        #
        self.map_grid_interpolator = RegularGridInterpolator((self.pedal_grid_map, self.velocity_grid_map), self.map_grid)
    ...
    def accel_input_to_pedal(self, accel_input, velocity):
        ...
        accel_input_map = alpha * self.map_grid[:,vel_idx] + (1 - alpha) * self.map_grid[:,vel_idx + 1]
            ...
            if accel_input_map[i] <= accel_input_map[i+1]:
                if accel_input_map[i] <= accel_input and accel_input <= accel_input_map[i+1]:
                    ...
                    if abs(delta_accel_input) < 1e-3:
                        ...
                        actuation_input = beta * self.pedal_grid_map[i+1] + (1 - beta) * self.pedal_grid_map[i]
                ...
                if accel_input_map[i+1] <= accel_input and accel_input <= accel_input_map[i]:
                    ...
                    if abs(delta_accel_input) < 1e-3:
                        ...
                        actuation_input = sign * (beta * self.pedal_grid_map[i] + (1 - beta) * self.pedal_grid_map[i+1])
...
class AccelBrakeMapConverter:
    ...
    def convert_actuation_cmd_to_accel_input(self, actuation_cmd, velocity):
        ...
        if actuation_cmd > 0.0:
            accel_input = self.accel_map_converter.pedal_to_accel_input(abs(actuation_cmd), velocity)
        if actuation_cmd <= 0.0:
            accel_input = self.brake_map_converter.pedal_to_accel_input(abs(actuation_cmd), velocity)
