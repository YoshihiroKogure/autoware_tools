    ...
    for i, t in enumerate(t_array):
        ...
        if t <= circle_AB:
            ...
            tangent_x = (
                ...
                * (theta_1_end - theta_1_start)
                / abs(theta_1_end - theta_1_start)
            )
            tangent_y = (
            ...
            tangent_x = (
                np.cos(np.pi / 2 + t3_rad)
                * (theta_2_end - theta_2_start)
                / abs(theta_2_end - theta_2_start)
            )
            tangent_y = (
                np.sin(np.pi / 2 + t3_rad)
                * (theta_2_end - theta_2_start)
                / abs(theta_2_end - theta_2_start)
            )
            yaw[i] = math.atan2(tangent_y, tangent_x)
...
def calc_two_circles_excircle_trajectory(
    ...
    for i, t in enumerate(t_array):
        ...
        if t <= circle_AB:
            ...
            tangent_x = (
                np.cos(np.pi / 2 + t2_rad)
                * (theta_excircle_end - theta_excircle_start)
                / abs(theta_excircle_end - theta_excircle_start)
            )
            tangent_y = (
                np.sin(np.pi / 2 + t2_rad)
                * (theta_excircle_end - theta_excircle_start)
                / abs(theta_excircle_end - theta_excircle_start)
            )
...
class calc_trajectory:
    def __init__(self, R):
        ...
        self.R = R
        L = 2.79
        self.steer_list = [-1.0, -0.90, -0.80, -0.70, -0.60, -0.50, -0.40] + [
            0.40,
            0.50,
            0.60,
            0.70,
        ...
        for i in range(len(self.steer_list)):
            ...
            (
                ...
                out_direction,
                total_distance,
            ) = calc_two_circles_common_tangent_trajectory(self.R, r)
            ...
            trajectory_reversed = {
                ...
                "parts": parts,
                "achievement_rates": achievement_rates,
                "in_direction": in_direction_reversed,
                "out_direction": out_direction_reversed,
                "total_distance": total_distance,
                "reversing":False
            }
            self.steer_trajectory_counter_clock_wise[
        ...
        for i in range(100):
            ...
            (
                ...
                _,
                total_distance,
            ) = calc_two_circles_common_tangent_trajectory(self.R, r, amplitude=amplitude)
            trajectory = {
                "type": boundary,
                "trajectory": traj,
                "yaw": yaw,
                ...
                "out_direction": "counter_clock_wise",
                "total_distance": total_distance,
                "reversing":True
            }
            ...
            trajectory_reversed = {
                ...
                "parts": parts,
                "achievement_rates": achievement_rates,
                "in_direction": "counter_clock_wise",
                "out_direction": "clock_wise",
                "total_distance": total_distance,
                "reversing":True
            }
        ...
        (
            ...
            _,
            total_distance,
        ) = calc_boundary_trajectory(self.R)
        trajectory = {
            "type": boundary,
            "trajectory": traj,
            "yaw": yaw,
    ...
    def add_trajectory(self, trajectory):
        ...
        modified_trajectory = {
            ...
            "trajectory": modified_traj,
            "yaw": modified_yaw,
            "curve": trajectory["curve"],
            "parts": trajectory["parts"],
            "achievement_rates": trajectory["achievement_rates"],
            "in_direction": trajectory["in_direction"],
            "out_direction": trajectory["out_direction"],
            "total_distance": trajectory["total_distance"],
            "reversing": trajectory["reversing"],
        }
    ...
    def get_reversal_loop_circle_trajectory_points(self):
        trajectory_position_data = np.concatenate(
            [self.trajectory_list[i]["trajectory"] for i in range(3)],
            axis=0,
        )
        trajectory_yaw_data = np.concatenate(
            [self.trajectory_list[i]["yaw"] for i in range(3)],
            axis=0,
        )
        trajectory_longitudinal_velocity_data = np.zeros(len(trajectory_yaw_data))
        trajectory_curvature_data = np.concatenate(
            [self.trajectory_list[i]["curve"] for i in range(3)], axis=0
        )
        trajectory_achievement_data = np.concatenate(
            [self.trajectory_list[i]["achievement_rates"] for i in range(3)], axis=0
        )
